...
bmap(struct inode *ip, uint bn)
{
  ...
  bn -= INDEX_NUM_1;
  if (bn < INDEX_NUM_2) //二级间接块
  {
    if ((addr = ip->addrs[INDEX_ENTRY_2]) == 0)
      ip->addrs[INDEX_ENTRY_2] = addr = balloc(ip->dev);
    struct buf *bp2 = bread(ip->dev, addr);
    uint *a2 = (uint *)bp2->data; //拿到二级间接块缓存
    int index = bn / INDEX_NUM_1; //算出在哪一个一级间接块的当中
    bn %= INDEX_NUM_1;            //算出在一级间接块的index
    if ((addr = a2[index]) == 0)
    {
...
void itrunc(struct inode *ip)
{
  ...
  {
    struct buf *bp2 = bread(ip->dev, ip->addrs[INDEX_ENTRY_2]);
    uint *a2 = (uint *)bp2->data;
    for (i = 0; i < INDEX_ENTRY_1; i++)
    {
      if (a2[i])
      {
